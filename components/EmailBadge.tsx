...
export function EmailBadge({ email }: { email: string }) {
  ...
  return (
    <div 
      onClick={handleCopy}
      className="flex items-center justify-center px-[7px] py-[2px] gap-[7px] bg-[#F5F5F5] border-[1px] border-dashed border-[#D7D7D7] rounded-[3px] cursor-pointer hover:bg-[#EAEAEA] transition-colors"
      title="Copy to clipboard"
    >
      <span className="font-mono text-[13px] text-[#9E9E9E]">{email}</span>
